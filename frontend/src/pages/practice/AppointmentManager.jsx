...
const STATUS_OPTIONS = ['scheduled', 'confirmed', 'arrived', 'in_progress', 'done', 'no_show', 'cancelled']
...
export default function AppointmentManager({ practiceId }) {
  ...
  useEffect(() => {
    if (!socket) return
    socket.on('appointment_updated', () => fetchAppts())
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {appointments.map(appt => (
          <div key={appt.id} className={`card border-l-4 ${
            appt.status === 'in_progress' ? 'border-l-green-500' :
            appt.status === 'arrived' ? 'border-l-teal-500' :
            appt.status === 'no_show' ? 'border-l-red-400' :
            appt.status === 'cancelled' ? 'border-l-gray-300' :
            appt.status === 'done' ? 'border-l-gray-400' : 'border-l-blue-400'
          }`}>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="text-sm text-gray-500 mt-0.5">
                  🕐 {format(new Date(appt.scheduled_at), 'h:mm a')} · {appt.duration_minutes} min
                </div>
            ...
            {!['done', 'cancelled'].includes(appt.status) && (
              <div className="flex gap-2 mt-3 flex-wrap">
                {appt.status === 'scheduled' && (
                  <button onClick={() => updateStatus(appt.id, 'confirmed')} className="btn-secondary text-xs px-2 py-1">✓ Confirm</button>
                )}
                {['scheduled', 'confirmed'].includes(appt.status) && (
                  <button onClick={() => updateStatus(appt.id, 'arrived')} className="bg-teal-600 text-white text-xs px-2 py-1 rounded-lg hover:bg-teal-700">🚶 Arrived</button>
                )}
                {appt.status === 'arrived' && (
                  <button onClick={() => updateStatus(appt.id, 'in_progress')} className="btn-success text-xs px-2 py-1">▶ Start</button>
                )}
                {appt.status === 'in_progress' && (
                  <button onClick={() => updateStatus(appt.id, 'done')} className="btn-success text-xs px-2 py-1">✓ Done</button>
                )}
                {!['done', 'no_show', 'arrived', 'in_progress'].includes(appt.status) && (
                  <button onClick={() => updateStatus(appt.id, 'no_show')} className="btn-danger text-xs px-2 py-1">✗ No-show</button>
                )}
                {!['done', 'cancelled', 'in_progress'].includes(appt.status) && (
                  <button onClick={() => updateStatus(appt.id, 'cancelled')} className="text-gray-400 text-xs px-2 py-1 hover:text-gray-600">Cancel</button>
                )}
              </div>
            )}
          </div>
